...
resource "yandex_compute_instance" "build" {
  ...
  provisioner "local-exec" {
    command = "pwd && ansible-playbook -u ubuntu -i ${self.network_interface[0].nat_ip_address} --private-key ${yandex_compute_instance.build.metadata.ssh-keys} ansible/main.yml"
  }
}
